...
- [ ] заменить собственные отображаемые строки BuildWorks стабильными языковыми ключами;
- [ ] сделать английский runtime-языком по умолчанию и добавить полный русский каталог;
- [ ] автоматически следовать текущему языку Valheim с явным BepInEx override при необходимости;
- [ ] сделать внутренние идентификаторы категорий и материалов независимыми от языка;
- [ ] локализовать контекстные подсказки, ошибки, параметры инструментов, каталог, дерево, HUD и метаданные adaptive pieces;
- [ ] добавить проверки отсутствующих ключей, fallback, аргументов форматирования и полноты English/Russian;
- [ ] проверить оба языка в Unity Workbench, затем в Valheim.
